...
def _get_ratings_for_products(product_ids: list[int]) -> list[dict]:
...
get_product_rating = tool(_get_product_rating)
get_ratings_for_products = tool(_get_ratings_for_products)


if __name__ == "__main__":
    # Single product
    result = _get_product_rating(1)
    print("Single product rating:")
    print(f"  Product {result['product_id']}: {result['average_rating']} stars ({result['review_count']} reviews)")

    # Multiple products
    print("\nBatch ratings:")
    results = _get_ratings_for_products([1, 3, 5, 7])
    for r in results:
